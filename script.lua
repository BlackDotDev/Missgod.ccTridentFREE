...
do
    ...
    local Silent = Tabs.main:createGroup('left', 'Rage(Use At Own Risk)')
    
    Silent:addToggle({text = "Silent Aim(Use At Own Risk)",default = false,flag = "Silent Aim",callback = function(state)
        getgenv().SilentEnabled = state
    end})
    --- Manipulation --
    Silent:addToggle({text = "Manipulation(Use At Own Risk)",default = false,flag = "Manipulation",callback = function(state)
    ...
    local HBE = Tabs.main:createGroup('right', 'Hitbox|Etc(Use At Own Risk)')
    ...
    HBE:addToggle({text = "Hitbox(Use At Own Risk)", default = false, flag = "Hitbox", callback = function(state)
        h.enabled = state
    end})  
    ...
    HBE:addToggle({text = "HitboxOverRider(Use At Own Risk)",default = false,flag = "hitboxoverrider",callback = function(state)
    ...
    Library:Notify("Missgod.cc - succesfully copied join code!", 5)
    end})
    gameTab:addButton({text = "rejoin",callback = function(Value)
    game:GetService('TeleportService'):TeleportToPlaceInstance(game.PlaceId, game.JobId)
    library:Notify("Missgod.cc - succesfully rejoining server!", 5)
    end})
    end
